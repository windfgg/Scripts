  ...
  return new Promise((resolve) => {
    let opts = {
      url: `https://api.m.jd.com/api?functionId=getCoupons&appid=u&_=${Date.now()}&loginType=2&body={%22platform%22:4,%22unionActId%22:%2231134%22,%22actId%22:%22${
        $.actId
      }%22,%22d%22:%223dVdwMF%22,%22unionShareId%22:%22${shareId}%22,%22type%22:1,%22eid%22:%22${
        $.eid
          ...
          if (typeof res == "object") {
            ...
            if (res.code == 0 && res.data) {
              ...
              if (res.data.type == 1) {
                ...
                msg = `获得[优惠券]🎟️满${res.data.quota}减${
                  res.data.discount
                ...
                msg = `获得[打折券]]🎫满${res.data.quota}打${
                  res.data.discount * 10
                }折 使用时间:${$.time(
                  "yyyy-MM-dd",
                  res.data.beginTime
                )} ${$.time("yyyy-MM-dd", res.data.endTime)}`;
              } else {
                msg = `获得[未知]🎉${res.data.quota || ""} ${
                  res.data.discount
                } 使用时间:${$.time("yyyy-MM-dd", res.data.beginTime)} ${$.time(
                  "yyyy-MM-dd",
                  res.data.endTime
  ...
  return new Promise((resolve) => {
    let opts = {
      url: `https://api.m.jd.com/api?functionId=shareUnionCoupon&appid=u&_=${Date.now()}&loginType=2&body={%22unionActId%22:%2231134%22,%22actId%22:%22${
        ...
        $.shareCode
      }%22,%22d%22:%223dVdwMF%22,%22supportPic%22:2,%22supportLuckyCode%22:0,%22eid%22:%22${
        $.eid
        ...
        if (err) {
          ...
          if (typeof res == "object") {
            if (res.code == 0 && res.data && res.data.shareUrl) {
              $.shareCode =
                (res.data.shareUrl.match(/3dVdwMF\?s=([^&]+)/) &&
                  res.data.shareUrl.match(/3dVdwMF\?s=([^&]+)/)[1]) ||
                "";
...
function getUrl() {
  return new Promise((resolve) => {
    const options = {
      url: `https://u.jd.com/3dVdwMF?s=${$.shareCode}`,
      followRedirect: false,
      headers: {
        Cookie: `${cookie} ${newCookie}`,
        "User-Agent": $.UA,
